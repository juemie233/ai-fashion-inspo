...
function getColumns() {
  return [
    { title: '平台', dataIndex: 'platform', width: 90, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return PLATFORM_LABELS[r.platform] || r.platform } },
    { title: '关键词', dataIndex: 'keywords', ellipsis: true, tooltip: true, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return h('span', [
      h('span', r.keywords.join(', ') || '-'),
      r.keywords.length > 1 ? h('span', { style: { marginLeft: '6px' } }, [h(Tag, { size: 'small', color: 'arcoblue' }, { default: () => '轮换' })]) : null,
    ]) } },
    { title: '数量', dataIndex: 'max_count', width: 60 },
    { title: '间隔', dataIndex: 'interval', width: 100, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return intervalLabel(r.interval_minutes) } },
    { title: '排序', dataIndex: 'sort_mode', width: 70, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return r.platform === 'xiaohongshu' ? SORT_MODE_LABELS[r.sort_mode || 'general'] : '-' } },
    { title: '下次执行', dataIndex: 'next_run_at', width: 150, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return formatDate(r.next_run_at) } },
    { title: '上次执行', dataIndex: 'last_run_at', width: 150, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return formatDate(r.last_run_at) } },
    { title: '已执行', dataIndex: 'run_count', width: 65 },
    { title: '启用', dataIndex: 'enabled', width: 70, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return h(Switch, { modelValue: r.enabled, size: 'small', loading: togglingId.value === r.id, onChange: () => toggleSchedule(r) }) } },
    { title: '操作', dataIndex: 'actions', width: 210, render: ({ record }: { record: unknown }) => { const r = record as ScraperSchedule; return h('span', { style: { display: 'flex', gap: '4px' } }, [
      h(Button, { size: 'mini', type: 'primary', loading: runningId.value === r.id, onClick: () => runNow(r) }, () => '立即执行'),
      h(Button, { size: 'mini', onClick: () => openEdit(r) }, () => '编辑'),
      h(Popconfirm, { content: '确定删除此定时计划？', onOk: () => deleteSchedule(r) }, { default: () => h(Button, { size: 'mini', status: 'danger', loading: deletingId.value === r.id }, () => '删除') }),
    ]) } },
  ]
}
</script>
...
<div>
  <a-card title="新建定时计划" size="small" style="margin-bottom:16px">
    <a-form :model="{ formPlatform, formKeywords, formMaxCount, formSortMode, formInterval, formEnabled }" label-align="left" :label-col-style="{ width: '80px' }" size="small">
  ...
  <a-modal v-model:visible="showEdit" title="编辑定时计划" :footer="false" :width="480">
    <a-form :model="{ editPlatform, editKeywords, editMaxCount, editSortMode, editInterval }" label-align="left" :label-col-style="{ width: '80px' }" size="small">
      <a-form-item label="平台">
    ...
    <template #footer>
      ...
      <a-button size="small" type="primary" :loading="updatingId !== null" @click="submitEdit">保存</a-button>
    </template>
  </a-modal>
</div>
</template>
