...
const emit = defineEmits<{
  ...
  /** 点击校验：父级执行 Cookie 真实登录态校验 */
  (e: 'verify-cookie', platform: string): void
}>()
...
/** 真实校验结果的徽标文案与颜色 */
function verifyBadge(cs: CookieStatus): { text: string; color: string } | null {
  const v = cs.verify
  if (!cs.exists || !v) return null
  if (v.state === 'valid') return { text: '已验证有效', color: 'green' }
  if (v.state === 'invalid') return { text: '已失效', color: 'red' }
  if (v.state === 'no_file') return null
  return { text: '未确认', color: 'orange' }
}
</script>
...
<template>
  ...
  <div class="cookie-cards">
    <a-card
      v-for="(cs, plat) in cookieStatuses"
      :key="plat"
      size="small"
    ...
    >
      <template #title>
        ...
        <a-tag :color="cs.valid ? 'green' : 'red'" size="small" style="margin-left: 8px">{{
          cs.exists ? (cs.valid ? '有效' : '已过期') : '未配置'
        }}</a-tag>
        <a-tooltip v-if="verifyBadge(cs)" :content="cs.verify?.detail">
          <a-tag :color="verifyBadge(cs)!.color" size="small" style="margin-left: 4px">{{
            verifyBadge(cs)!.text
          }}</a-tag>
        </a-tooltip>
      </template>
      ...
      <p
        v-if="cs.verify && cs.verify.state !== 'valid'"
        style="font-size: 12px; color: #f53f3f; margin: 4px 0 0"
      >
        {{ cs.verify.state === 'invalid' ? '⚠ 已确认失效：' : '⚠ 未确认：' }}{{ cs.verify.detail }}
      </p>
      <template #actions>
        <a-space size="small">
          ...
          <a-button
            ...
            size="mini"
            type="outline"
            :loading="verifyingCookie === plat"
            @click="emit('verify-cookie', plat)"
            >校验</a-button
          >
          <a-popconfirm
            v-if="cs.exists"
          ...
          >
            <a-button size="mini" type="outline" status="danger" :loading="deletingCookie === plat"
              >删除</a-button
            >
          </a-popconfirm>
        </a-space>
  ...
  <!-- Cookie 导入对话框 -->
  <a-modal
    :visible="showCookieImport"
    @update:visible="onUpdateCookieImportShow"
    title="导入 Cookie"
    :width="500"
    :footer="false"
  >
    <p style="font-size: 12px; color: #666; margin-bottom: 8px">
      粘贴 {{ PLATFORM_LABELS[cookiePlatform] || cookiePlatform }} 的 Cookie JSON 数据
    </p>
    <a-textarea
    ...
    />
    <a-button type="primary" long style="margin-top: 12px" @click="emit('import-cookie')"
      >导入</a-button
    >
  </a-modal>
  ...
  <a-card title="可用采集源" style="margin-bottom: 16px" size="small">
    <a-list>
      <a-list-item v-for="src in sources" :key="src.platform">
        <template #extra>
          <a-tag :color="src.status === 'available' ? 'green' : 'orange'" size="small">{{
  ...
  <a-card size="small" style="margin-bottom: 16px">
    <div
      class="tombstone-header"
      @click="emit('update:showTombstone', !showTombstone)"
    ...
    <div v-if="showTombstone" class="tombstone-body">
      <p style="margin: 8px 0 0; font-size: 12px; color: #666">
        📌 墓碑表记录了所有曾下载过的图片 URL，采集时自动跳过，确保永不重复入库。当前共
        <b>{{ tombstoneCount }}</b> 条。
      </p>
    </div>
  </a-card>
  ...
  <div
    v-for="hint in sources.map((src) => {
      let level: 'warning' | 'info' | 'error' = 'info',
        tips: string[] = []
      if (src.platform === 'xiaohongshu') {
        level = 'warning'
        tips = ['需要有效的登录 Cookie', '反爬检测严格', '搜索功能依赖页面 DOM 结构']
      } else if (src.platform === 'douyin') {
        level = 'error'
        tips = ['网页版功能严重受限', '搜索结果可能为空', '推荐使用浏览器插件代替']
      } else {
        tips = ['最可靠的采集方式', '支持一键抓取当前页面']
      }
  ...
  >
    ...
    >
      <ul style="margin: 4px 0; padding-left: 18px; font-size: 13px">
        <li v-for="t in hint.tips" :key="t">{{ t }}</li>
      </ul>
    </a-alert>
  </div>
</template>
