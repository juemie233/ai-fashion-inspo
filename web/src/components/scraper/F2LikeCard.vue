 ...
 *    「抖音入库自动收藏」下的二级收藏夹（每个抖音收藏夹一个）；结果审查在「抖音采集历史」里
 */
...
const copy = computed(() =>
  isCollect.value
    ...
    : {
        ...
        listName: '点赞列表',
        speedTip:
          'f2 的点赞分页没有「遇到已下载就停」，从最新一路翻到底、每页还固定等一次 ' +
  ...
  <a-card :title="copy.title" size="small" style="margin-bottom: 16px">
    <div class="f2l-row">
      ...
      <template v-if="isCollect">
        ...
        <a-popconfirm
          content="这会下载并入库全部收藏夹的内容（含股票、哲学这类与穿搭无关的夹）。建议改用「先扫描收藏夹」只勾想要的夹 —— 那样未勾选的夹既不会下载，也不会入库。"
          ok-text="仍然全部下载"
          cancel-text="改用勾选"
    ...
    <div class="f2l-tips">
      <div>
        · {{ copy.listName }}只有本人可见：这里填<b>你自己</b>的主页链接（网页版打开你的主页，地址栏
      ...
      <div>
        ...
        <b>根本不读 <code>-i</code></b
        >（源码实测），且分页没有「遇到已下载就停」——所以提速只能靠上面的
    ...
    >
      ...
      <div class="f2l-option-tip f2l-collect-foot">
        · 下载落点、命名模板与判重口径都与原来一致；入库后只收<b>勾选的这些夹</b>的作品，
        未勾选的夹即使文件已在硬盘上也不会入库。每个夹最多取「每次最多翻」条（0 = 该夹全量）。
        <br />
        · <b>已入库的作品不会重复下载</b>：素材库里已有的整件跳过、另一个列表目录里
        已有的文件直接复用（省一次下载），但这些作品仍会补进对应的二级收藏夹。
        <br />
