...
/** 素材人脸识别区块：触发检测与模特特征库匹配，展示结果，支持手动指定/解除模特。
 *
 * 逻辑：检测结果（inspiration_face_detections）→ 匹配到的模特 / 疑似未知人脸
 * （低于阈值未匹配）→ 用户可手动选择模特或解除错误关联。
 */
...
import { modelsApi, type PersonBrief } from '@/api/persons'
...
async function loadModelOptions() {
  try {
    const { items } = await modelsApi.fetchList({ sort: 'count', size: 100 })
    modelOptions.value = (items as PersonBrief[]).map((m) => ({
      label: m.name,
      value: m.id,
    }))
  } catch {
...
async function handleDetect() {
  ...
  try {
    ...
    if (data.face_count === 0) {
      ...
      message.success(`检测到 ${data.face_count} 张人脸，已与模特特征库匹配`)
    }
  } catch (e) {
...
/** 手动指定模特（v-model 变更触发） */
async function handleSelectModel(det: FaceDetectionOut, modelId: number | null) {
  if (modelId === null) return // n-select clear 时不处理
  updatingId.value = det.id
  try {
    await updateFaceDetection(props.inspirationId, det.id, modelId)
    message.success('已关联模特')
    await load()
  } catch (e) {
...
<template>
  <div class="face-detection-section">
    <div class="face-header">
      <h3 style="margin: 0">人脸识别（模特特征库匹配）</h3>
    ...
    <div v-else-if="detections.length === 0" class="face-tip">
      暂无检测结果。点击「检测并匹配」识别图中人脸并关联职业模特（需先在模特详情页注册人脸）。
    </div>
    ...
    <div v-else class="face-list">
      <div v-for="det in detections" :key="det.id" class="face-item">
        ...
        <n-tag
          v-if="det.matched_model_id !== null"
          type="success"
          size="small"
          :bordered="false"
        >
          {{ det.matched_model_name ?? `模特 #${det.matched_model_id}` }}
        ...
        <n-select
          ...
          :value="det.matched_model_id"
          :disabled="updatingId === det.id"
          @update:value="(v: number | null) => handleSelectModel(det, v)"
        />
        <n-button
          v-if="det.matched_model_id !== null"
          size="tiny"
