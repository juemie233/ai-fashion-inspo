...
if errorlevel 1 (
  ...
  echo   [!]  警告：端口 %BACKEND_PORT% 可能仍被占用
)
...
if errorlevel 1 (
  ...
  echo   [!]  警告：端口 %FRONTEND_PORT% 可能仍被占用
)
...
if not exist "face-service\.venv\Scripts\python.exe" (
  echo   [!]  未检测到 face-service\.venv（Python 3.10 虚拟环境），跳过人脸服务
  set FACE_REQUIRED=0
) else (
...
if !BACKEND_OK!==1 (
  ...
  echo   [X] 后端未就绪，请检查 %LOG_DIR%\backend.log
)

if !FRONTEND_OK!==1 (
  echo   [OK] 前端已就绪: http://localhost:%FRONTEND_PORT%
) else (
  echo   [X] 前端未就绪，请检查 %LOG_DIR%\frontend.log
)
...
:face_ready
if "!FACE_OK!"=="1" (
  echo   [OK] 人脸服务已就绪: http://localhost:%FACE_PORT%
) else (
  echo   [X] 人脸服务未就绪，请检查 %FACE_LOG%
)
goto :face_done
:face_skip
echo   [--]  人脸服务未部署（跳过验证）
...
if !WORKER_OK!==1 (
  echo   [OK] worker 已启动（日志: %LOG_DIR%\worker.log）
) else (
  echo   [!]  worker 可能未就绪，请检查 %LOG_DIR%\worker.log
)
