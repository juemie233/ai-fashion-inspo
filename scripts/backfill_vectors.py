...
async def main() -> None:
    ...
    async with async_session() as db:
        stats = await backfill_all_vectors(db, mode=args.mode, limit=args.limit)

    if "error" in stats:
        print(f"[错误] {stats['error']}")
        sys.exit(1)

    print(f"回填完成（mode={args.mode}, limit={args.limit}）:")
    for key in ("processed", "text_added", "text_failed", "image_added", "image_failed", "skipped_non_image"):
