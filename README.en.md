...
| `POST` | `/api/admin/near-duplicates` | Near-duplicate detection (full-library random sampling + perceptual hash grouping; only returns candidates, doesn't delete; hashes are cached into `inspirations.phash` after the first computation, missing hashes are progressively backfilled per request) |
