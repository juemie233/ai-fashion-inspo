...
| **Inspiration Detail** | Large image preview (lightbox left/right switching/zoom), tag display, outfit master tags (manual select/create + one-click import of AI suggestions), similar inspiration recommendations (favorite/delete), re-analysis, download original image, copy original link, click a tag to jump to search |
...
| ------ | ------ | ------ |
| `GET` | `/api/inspirations` | Inspiration list (paginated; filters: source/media/status/quality/tag/dominant color/date; sorting includes `tag_count`) |
...
| `POST` | `/api/inspirations/{id}/trash` | Move to trash (soft delete; `reason` optional: 质量差/重复/不喜欢/隐私/其他) |
