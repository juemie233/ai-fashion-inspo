...
async def test_read_image_base64_converts_webp_content_mislabeled_jpg():
    """WebP 内容存成 .jpg 扩展名：按实际格式识别并转 JPEG（修复 Ollama 400）。"""
    rel = _write_image("images/webp_as_jpg.jpg", "WEBP")
    image_data, _ = await _read_image_base64(rel)
    raw = base64.b64decode(image_data)
    assert raw[:3] == b"\xff\xd8\xff"  # JPEG SOI 魔数


async def test_read_image_base64_converts_webp_extension():
...
async def test_read_image_base64_keeps_jpeg_unchanged():
...
async def test_read_image_base64_keeps_png_unchanged():
    """真实 PNG：保留原字节。"""
...
async def test_read_image_base64_rejects_corrupt_image():
    ...
    with pytest.raises(ValueError, match="图片解析失败"):
        await _read_image_base64(rel_path)
