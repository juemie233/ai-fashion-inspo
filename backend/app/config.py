...
class Settings(BaseSettings):
    ...
    f2_like_user: str = ""
    # 「我的喜欢」每次最多翻多少条（传给 f2 的 `-o/--max-counts`）。0 = 全量翻到底。
    # 为什么需要：f2 的点赞分页**没有「遇到已下载就停」**，从 cursor=0 一路翻到底，
    # 且每页固定 sleep 一次 timeout（本机 10 秒）——点赞目录 919 个作品即 ≥46 页、
    # ≥7.7 分钟纯等待，**每次运行都一样，哪怕零新增**；同时进度条按「已落盘文件数」
    # 计算，零新增时它会全程停在 0，看起来像卡死。
    # 点赞列表最新在前，所以只翻最近 N 条就能覆盖新增：填 100~200 时日常增量降到
    # 一两页（秒级）。代价是「两次运行之间新增点赞超过 N 条」会漏，故默认 0（全量，
    # 行为与改造前一致），需要提速时再开。
    f2_like_max_counts: int = 0
