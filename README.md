...
| 模块 | 功能 |
| ------ | ------ |
| **素材库** | 瀑布流浏览、多维筛选（来源/媒体/状态/标签/主色调）、排序（含随机/标签数）、密度调节、分页加载、批量多选操作（收藏/移垃圾桶/加标签/编辑元数据）、**收藏合集**（手动合集：素材加入/移除/合集内拖拽编排；智能合集：按标签/筛选条件动态求值并支持一键固化为手动合集；合集封面/排序，垃圾桶素材不参与；**两级结构**：一级固定为「系统入库手动收藏」与「抖音入库自动收藏」，二级=抖音收藏夹，最多两级）、浏览模式/密度/每页数量持久化 |
| **高级搜索** | 关键词搜索、标签筛选(AND/OR)、共现推荐、高级筛选(来源/媒体/日期)、排序(匹配优先)、搜索历史、分页、密度调节、语义搜索（文本）、以图搜图（图片上传）、`/` 聚焦与 Esc 退出、复制搜索链接、筛选状态持久化 |
| **上传素材** | 拖拽/粘贴/URL导入、预览队列（视频可预览）、上传进度与速度、快速标签、元数据预设、去重检测、文件夹批量、队列管理（清空二次确认）、偏好设置持久化、1500 上限校验 |
| **素材详情** | 大图预览（灯箱左右切换/缩放）、标签展示、**AI 打标纠错反馈**（AI 标签悬停「标错了」→ AI 多标/类别错/名称不规范，多标会同时移除该标签；标签区「AI 漏标了？补充」→ 补建标签；反馈落纠错库供质量看板统计）、穿搭大标签（手动选择/新建 + AI 建议一键入库）、相似素材推荐（可收藏/删除）、重新分析、**视频关键帧条带**（视频素材懒提取缩略图，失败静默隐藏）、下载原图、复制原始链接、标签点击跳搜索、移入垃圾桶（**必选删除原因**：质量差/重复/不喜欢/隐私/其他/AI生成，单个与批量共用原因弹窗，未选禁用确认）、五星评分（与收藏并列，列表可筛选/排序）、**人脸识别（博主特征库匹配）**（检测并匹配 → 自动关联穿搭博主 / 疑似未知人脸 → 手动指定或解除关联，需先在博主详情页注册人脸） |
| **采集管理** | 小红书 CDP 零检测采集；**抖音 CDP 完整采集**（搜索三层策略——直连搜索页 / 搜索接口响应 / 首页搜索框兜底，反风控入口轮换自适应；搜索与按博主双模式；详情页图集多图 / 视频 / 正文 / 话题标签提取，视频下载入库，素材自动关联博主；登录 / 机器人验证感知，人工解决等待 + 诚实报错；任务结束后自动关闭采集新开的浏览器标签页）、**按博主采集**（选博主 → 进其主页逐篇打开笔记详情页，提取轮播多图/视频/正文 caption/话题标签，视频下载入库，素材自动关联博主）、任务分页/平台与状态筛选/排序、取消/续采（断点）/复制重采、日志查看、漏斗可视化（含跳过原因统计）、结果预览（批量删除/加载更多/跳详情）、Cookie 管理（状态/时效/导入/删除/**真实有效性校验**：探测平台登录态接口，失效在管理页显式标记，新建任务前置拦截已失效 Cookie）、Chrome 生命周期管理、定时采集（计划 CRUD/启停/立即执行）、**话题标签存档**（笔记话题自动入库：全局去重 + 累计出现次数 + 来源追溯，新建/编辑采集任务可从话题库点击复用为关键词）、统计看板（平台分布/每日趋势）、**采集 ROI 漏斗**（按关键词 / 博主拆解「发现 → 入库 → 合格率」，两个通道各自的真实口径见下）、URL 墓碑表 + 内容 MD5 去重、筛选/排序/页签持久化 |
...
| **标签高级管理** | 独立路由页（侧边栏「标签高级管理」入口）：**健康度分析**（孤儿/低频/低质命名/疑似重复/**不合规命名（裸词）**五类扫描 + 健康评分 + 一键修复；不合规命名按打标命名口径列出袜/鞋/裙裸词与缺长度丝袜，附原因与关联数，可勾选「合并到…」规范标签或批量删除/编辑）、**自动聚类**（名称相似 + 共现加成产出候选合并组，人工确认后合并/建别名）、**网络图分析**（Top-N 共现子图 + 社区发现 + 中心度 + 桥接节点高亮，点击节点看趋势）、**批量高级编辑**（正则查找替换/前后缀增删/格式归一化/正则批量合并，dry-run 预览 + 撞名自动合并）、**标签层级树**（parent_id 任意深度树，拖拽移动子树 + 循环检测）、**操作历史与回滚**（全部标签写操作快照，单条回滚 + 冲突检测）、**使用效果分析**（热度升降榜/标签组合/覆盖度/来源分布） |
...
| **人物管理** | **穿搭博主 / 职业模特双 Tab 独立管理**（两类已物理拆分为独立表与 API，业务逻辑各自演进）：列表（名称搜索/平台筛选/排序）、新建/编辑/删除（仅无关联素材时可删）、热门排行、风格画像（高频标签/类别分布/趋势）、**IP 属地统计**（按属地聚合博主数/素材数，横向条形图不隐藏任何一档标签）、素材关联（详情页按博主/模特分区块搜索添加/解除）、**博主 CSV 导入**（按小红书号 upsert）、**模特照片组**（选择文件夹整组导入到选定模特、照片组浏览/灯箱/删除、组内 SHA-256 去重）、**博主头像**（两种来源二选一：从 TA 已关联的素材里选一张（视频取首帧缩略图），或本地上传一张照片；取到照片后**提取人像**——送人脸子服务取最大人脸、按检测框外扩裁成 256×256 正方形，无脸或子服务不可用时回退整张照片；列表与详情按**圆形**展示，可一键清除）、**博主 人脸特征注册**（上传正脸照片 与/或 从已关联素材中选图，两种来源合计 1~5 张，注册/重新注册，素材人脸自动匹配依赖此特征库；**匹配阈值可按人单独配置**——未配置的走兜底阈值，匹配报告区分「兜底阈值 + 另 N 人按各自阈值判定」；**子服务离线态显式化**：离线时人脸动作禁用并给出提示，离线期间每 15 秒自动复检、也可点「重新检测」，启动子服务后不必刷新页面；职业模特无此人脸能力）、**注销人脸 / 解绑素材**（博主详情多选素材批量注销已匹配人脸、批量解除素材-博主关联，素材侧已确认人脸自动防重复匹配）、**一键补全主页（按平台分流）**（小红书：拉一次「我关注的用户」列表按昵称归一化解析 uid，补 `profile_url` / `platform_user_id`，其余走本地 URL↔ID 互推——**不按小红书号搜用户**，实测搜不到本人；抖音：离线读 f2 用户库按 `sec_user_id` 精确匹配回填 IP 属地，**只补空缺**；补不到的记入「已跳过」写明原因，可解除后重试）、**人物组（博主跨平台绑定）**（同一现实人物在抖音/小红书各有账号时绑定为同一人——如小红书「Fox_」与「多多」：列表同组折叠为一条主账号（素材数最多者，可手动指定默认展示位）并带多平台徽标，展开可见组内各账号及其素材，详情页可绑定/解绑/切主，账号记录全部保留、按平台采集不受影响） |
| **任务管理** | 聚合任务队列与采集任务统一查看：分页/状态与类型筛选（类型中文映射 + 图标 + 颜色区分：批量分析/组合分析/质量审核/批量删除/近似重复检测删除/采集/**抖音素材获取**/向量回填/标签高级管理）、进度条与完成统计（向量/删除/审核明细）、**运行中任务的阶段文案**（如 f2 获取素材的「第 4/21 个作者 · 逐作者翻页，单作者约 10 秒~4 分钟」与「第 512/11936 个文件 · 复制文件并生成缩略图」，避免长时间停在低百分比却没说明；「我的喜欢」下载期另显示实时计数）、**运行中任务暂停 / 已暂停恢复 / 运行中取消**（批量分析、组合分析、标签网络分析、质量审核支持——批量/组合暂停后恢复按「已成功跳过」幂等续算；可暂停 / 可取消的任务类型白名单前后端同一份、由测试锁定）、已暂停任务排到列表最前、取消排队任务、失败采集一键重试、**任务进度 WebSocket 实时推送**（断线自动重连 + 轮询降级兜底，侧边栏展示连接状态）、预计剩余时间、**数据备份状态卡片**（自动补备开关 / 备份进行中（双通道运行锁）/ 最近成功备份 / 历史记录（成功失败标记）/ 备份日志尾部，只读实时展示） |
| **抖音素材获取（f2）** | 独立的抖音采集通道（**不依赖 Chrome / CDP**，走本地 f2 按博主增量下载）：**一键获取素材**（调 f2 逐作者增量下载 → 五层去重 → 入库，一个按钮 / 一条命令）、**「我的喜欢」通道**（`mode=like`：只给「我的主页链接」就能把点赞过的作品全部拉回来，不逐作者、不加时间窗口——喜欢列表按点赞时间排序，f2 的 `-i` 按发布时间过滤会漏掉「最近点赞的老视频」；**下载期实时进度**：观察器每 2 秒统计下载目录的「已下载 N 个 / 已下载 X MB」，软进度从 1% 起步、按耗时缓慢逼近 35% 上限（900 秒约走完一半差距，永不到顶），长时间下载不再看起来卡死）、**「我的收藏」通道**（`mode=collection`：拉抖音收藏列表，跨作者、同样不加时间窗口；**先扫描、后下载**——先列出收藏夹与每个夹的作品数，勾选后只下选中的夹（夹默认全选，选择本地只保留最近一次），入库后按「作品 ID → 收藏夹」清单自动归位到二级收藏夹；下载前先过滤已入库与可从「我的喜欢」复用的作品，入库集合取并集）、**「我的喜欢 / 我的收藏」交叉作品自动合并为硬链接**（同一作品不再各存一份）、**入库后自动登记博主**（「我的喜欢」天然跨作者，入库后把未登记的来源作者补建成抖音博主并绑定本批素材；补建的博主标记为「自动登记」——算博主也算素材归属，但**不算已登记博主**、不进「一键获取素材」的下载白名单，在博主列表点「纳入追踪」才转正，也可以按批次结果页手动登记）、**每日自动获取**（开关 + 间隔小时可配，后台按最近一次任务创建时间判定到期；环境不可用 / 已有任务在跑 / 未到期自动跳过，默认关闭）、**日期窗口**（默认只让 f2 翻最近 14 天并按作者目录 mtime 自适应放大——f2 在 `-i all` 时会把作者全部历史翻完且每页固定等 timeout 秒，实测单作者 84% 的时间耗在翻页等待）、**增量哈希缓存**（落盘 `storage/f2_hash_cache.db`，实测 15,303 个文件全量哈希 142 秒 → 预热后 2 秒）、**垃圾桶判重**（丢进垃圾桶的内容不会被重新导入）、导入不做标签分析（素材以未打标状态入库）、批次清单 + 一键回滚、任务中心显示阶段与「第 N/M 个作者 / 文件」计数 |
...
## 最近更新（2026-09-20 ~ 09-24，46 个提交）

> 上一次刷新本文档是 `8a899a3`（09-20 22:13），本节汇总其后至 `48fe246` 的全部提交。
> 逐提交的详细决策与实测数据见 [`docs/decisions/`](docs/decisions/)（按周归档）。

### 收藏合集两级结构（09-23 ~ 09-24）

- **一级只有两个**：**系统入库手动收藏**（智能合集，按 `is_favorite` 求值）与**抖音入库自动收藏**（f2 自动维护）；**二级**挂在后者之下，名字与抖音侧收藏夹一一对应，由 f2 入库阶段按「作品 ID → 收藏夹」清单自动建立；**最多两级**，不允许套娃（迁移 `a7b8c9d0e1f2`，含同层唯一约束）。名字口径唯一定义在 `app/models/collection.py`，服务/导入/前端文案共用（`f2c15b2`）。
- 跟手修复：入库只看勾选的夹、一级永不装素材（`62a20e4`）；收藏模式必须手选夹（`3141897`）；上一轮审查发现的静默失败与并发缺陷 H1/H2/M1/M2 + L1~L5（`3e919b6`）；拖拽赋值移出模板，消除 IDE 的 2322 误报（`7abfc90`）。

### 抖音收藏（f2）通道增强（09-22）

- **一键获取我的收藏**：把抖音收藏列表整表拉回并聚合进「抖音入库自动收藏」（`7c54b98`）。
- **先扫描、后下载**：先列出收藏夹（`GET /api/scraper/f2-collects`），勾选后只下选中的夹——`-M collects` 是交互式的、worker 里跑不了，改用 `collects/list` + `collects/video/list` 自建分页（`2847440`）；夹**默认全选**，选择记录本地只保留最近一次（`87aa157`）。
- **下载前过滤 + 收藏全集补齐（F + E）**：已入库 / 可从「我的喜欢」复用的作品不再重复下载，入库集合取「本批导入 + 既有素材」的并集（`c3b676f`）。
- **「我的喜欢 / 我的收藏」交叉作品自动合并为硬链接**，同一作品不再各存一份（`4489b76`）。
- **每日自动获取**新增「我的喜欢 / 我的收藏」入口（`3f1d269`）。
- 期间修掉：先扫描后下载的 4 处缺陷（含一处线程安全隐患，`d43b61a`）、CI 缺 f2 时报 `ModuleNotFoundError`（`92a3528`）、F+E 的 3 处（集合重建 / 静默截断 / SQLite 方言未标注，`78fef40`）、测试误扫真实下载目录（`2c3fed8`）、端到端用例与过期文档（`a996748`）、「我的喜欢」卡片「用户是否改过输入」标志改 `ref`（消除模板赋值的 2322 报错，`c43905e`）。

### 质量审核可暂停 / 恢复 / 取消（09-22）

暂停/恢复与标签分析共用同一套断点语义（`bac5775`），运行中可取消（`8e98ca3`）；前后端「可暂停 / 可取消」任务类型白名单由测试锁定，不再各写各的。

### 人脸识别增强（09-23 ~ 09-24）

- **按人自适应阈值**：可为每个博主 / 模特单独设人脸匹配阈值；匹配报告新增 `default_threshold` / `custom_threshold_count` / `threshold_mode`，页面显示「兜底阈值 X（另 N 人按各自阈值判定）」，不再把兜底值误读成本次匹配的唯一阈值（`39a5998`；迁移 `0bc72b4e2fa0`）。
- **子服务离线态显式化**：离线时人脸动作禁用并给出可操作提示，提示条带「重新检测」按钮，离线期间每 15 秒自动复检——启动 face-service 后不必刷新页面（`39a5998` + `8fd7c09`）。
- **扫描性能 3.4 倍**：face-service 的解码 + 推理改到有界线程池（此前同步调用直接阻塞 uvicorn 事件循环，主后端并发再多也被串行化），主后端并发 `32×4` → `8×8`（一批 64 个素材要切出 8 个请求才喂得满）。实测 8 路并发由 **17.4 张/秒 → 51~56 张/秒**；顺带修掉「空字节让整批 503」的缺口（`48fe246`）。
- 顺带修：聚合分组悬停大图把视频的 mp4 当图片加载导致浮层空白（`cc73f0f`）。

### 数据盘迁移与可配置存储根（09-22 ~ 09-23）

- `STORAGE_ROOT` / `F2_DOWNLOAD_ROOT` 进 `.env`，素材存储与 f2 产物根不再写死（`d8749a3`）；新增只读校验脚本 `verify_storage_move.py`（DB 里全部路径引用可达 + 逐目录文件数/字节比对，`f1a91d8`）；测试中过期的 f2 产物根断言同步修正（`881eec4`）；素材库路径收敛为单一口径，f2 判重与入库不再打到空库（`bdc18b7`）。
- **备份脚本两个真缺陷**：中断残留的过期锁让备份整整 7 天没跑成（现按锁目录 mtime 自愈并打印原因）；存储根迁移后脚本仍写死 `backend/storage`，会静默备份旧树、日志也写错位置（现跟随 `settings.storage_root`）（`56febbd` + `88b714c`）。

### 数据治理与任务管理（09-20 ~ 09-24）

- 近似重复对比：逐组上下翻页、决定可改（`d5e27d4`）；中途退出不再丢已做的删除决定（`a54a830`、`730af0d`，「对比完好像没删」）；确认删除后不再被弹回选择页（`711af2e`）；重复时保留副本优先选「有来源链接」的那条（`bceb810`）。
- 内容边界检测（抖音截图）扫描改为两段，解决「连轮零候选」（`8facd99`）；手机图剪裁命令行入口不再被「只处理手动上传素材」的来源限制挡住（`28f0165`）；哈希补齐任务进度回调未 `await` 导致进度全程停在 0%（`003efbc`）；f2 导入同作品同分段重复入库（批次内判重漏判，`35903b5`）。
- 任务管理：已暂停任务排到列表最前（`73f569a`）；「查看已登记博主」支持按总作品 / 已入库素材排序（`8f1dba1`）。
- AI 提示词：`qwen3-vl:8b-instruct` 明确堆堆袜与中筒袜的区别（`34b08c8`）。

### 工程与索引（09-20 ~ 09-23）

- `scripts/import_f2_downloads.py`（约 1.9k 行）拆成 8 个子模块 + 转发壳（`82c466f`）。
- OpenViking 同步：改为增量上传并排除测试（`b0b95c5`）；修 dry-run 落盘、过期周文件与日志轮转（`6629729`）；新增每日增量同步包装 `scripts/sync_openviking_task.bat` + 从 git log 生成决策台账（`5072fb9`）；同步前预热 embedding 模型，消除冷启动失败窗口（`5ac5d51`）。
...
# 后端 .env
PORT=18888                   # 后端监听端口
STORAGE_ROOT=G:/fashion-inspo-storage  # 素材存储根（默认 backend/storage；改这里 images/thumbnails/videos/keyframes/lancedb 等全部跟随）
F2_DOWNLOAD_ROOT=E:/f2-downloads       # f2 下载产物根（默认见 backend/scripts/f2_common.py 的 DEFAULT_F2_DIR；换盘后判重与入库都按它算）
...
**「我的喜欢」通道：** 同一张卡片上另有「我的喜欢」入口——抖音的点赞列表只有本人可见，所以要先填一次**你自己的抖音主页链接**（`PUT /api/scraper/f2-like-user`，或界面上的输入框），之后点一下就把点赞过的作品全部拉回来。它与发布模式的差异是刻意的：不逐作者（喜欢天然跨作者）、不加日期窗口（喜欢列表按点赞时间排序，而 f2 的 `-i` 按发布时间过滤，窗口会漏掉「最近点赞的老视频」）、不做作者白名单（发布模式默认只处理能对上游记博主的账号）。下载过程中界面会显示**实时进度**（每 2 秒统计下载目录的「已下载 N 个 / 已下载 X MB」，并按已耗时放软进度上限），不会再长时间停在 0%。入库后会把本批**未登记的来源作者自动补建成抖音博主并绑定素材**（`register_bloggers`，默认开）：补建的博主标记为「自动登记」（`source=auto_collect`），算博主也算素材归属，但**不算已登记博主**——不会被发布模式的「一键获取素材」下载，在博主列表点「纳入追踪」才转正；批次结果页也可手动批量登记。

```bash
# 「我的喜欢」只有 API / 界面入口（CLI 的 --fetch 走发布模式，不含点赞）
curl -X PUT "http://127.0.0.1:3000/api/scraper/f2-like-user?like_user=https://www.douyin.com/user/<我的 sec_user_id>"
curl -X POST "http://127.0.0.1:3000/api/scraper/f2-import?fetch=true&mode=like"
```
...
> **博主资料回填：** f2 的用户库（`douyin_users.db` 的 `user_info_web` 表）存有 `sec_user_id` / 昵称 / 作品数，是抖音博主 ID 的权威来源。`python -m scripts.sync_blogger_ids.py --file 名单.txt [--apply] [--create-missing] [--bind-materials]` 可按「昵称 + sec_user_id」清单回填博主 ID 与主页 URL，并把已入库素材按来源作者绑定到对应博主（缺省只预览）。界面上还有更省事的 **「一键补全主页」**（人物管理页）：抖音博主缺失的 IP 属地直接从这个库离线回填，小红书博主缺失的主页链接 / 平台用户 ID 走一次「我关注的用户」列表按昵称解析——详见 [穿搭博主 API](#穿搭博主-apibloggers) 下方的说明。
...
- **增量上传**：只上传有变化的文件（对比远端摘要），并排除测试文件；重复执行的开销从「全量重传」降为「按需补传」。
- **每日增量同步**：`scripts/sync_openviking_task.bat` 供 Windows 任务计划每日调用；同时从 `git log` 生成决策台账（`docs/decisions/` 按周归档），供语义检索回溯「某个改动为什么这么做」。
- 同步前预热 embedding 模型，消除首次同步的冷启动失败窗口；dry-run 结果落盘、过期周文件与日志自动轮转。
...
│   │   │   ├── tag_correction.py # AI 打标纠错记录（「标错了」反馈）
...
│   │   │   ├── person_service.py # 人物转发层 → person/（base 基类 + services + csv_import + photo_sets + person_groups + avatar 头像）
│   │   │   ├── tag_correction_service.py # 打标纠错反馈（记录 + 按原因删/补关联 + 统计）
│   │   │   ├── collection_service.py # 收藏合集（手动/智能求值/固化/排序）
│   │   │   ├── scraper_service.py     # 采集转发层 → scraper/（tasks 任务 + process 编排 + cookies/schedules/extension/results + roi 采集 ROI）
│   │   │   ├── blogger_face.py   # 博主人脸注册（平均池化）+ 素材人脸检测匹配
│   │   │   ├── blogger_enrichment_service.py # 博主主页资料补全（小红书关注列表解析 uid；抖音 f2 离线回填 IP 属地；跳过表）
│   │   │   ├── face_image.py     # 送检图片准备（格式归一 + 视频取首帧缩略图）
│   │   │   ├── face_crop.py      # 人像裁切（取最大人脸按框外扩 → 256×256 头像）
...
│   │   │   ├── task_runners/     # 异步任务执行器（f2_import / enrich_blogger_profile / batch_analyze / quality_check / batch_delete / deduplicate / vector_backfill / face_scan / face_cluster / tag_health / tag_cluster / tag_graph）
...
│   │   ├── import_f2_downloads.py # f2 抖音下载目录 → 素材库（转发壳；实现已拆到 f2_*.py）
│   │   ├── f2_common.py           # f2 通道公共：下载根解析 / 五层判重 / 个人重复合并
│   │   ├── f2_plan.py             # 扫描与计划（只读报表口径）
│   │   ├── f2_fetch.py            # 调 f2 下载（按博主 / 我的喜欢 / 我的收藏）
│   │   ├── f2_collects.py         # 抖音收藏夹：枚举（collects/list）与按夹下载
│   │   ├── f2_apply.py            # 入库（复制/缩略图/去重/批次清单）
│   │   ├── f2_report.py           # 批次报表与结果摘要
│   │   ├── f2_rollback.py         # 批次回滚（默认只预览，--apply 才删）
│   │   ├── f2_hash_cache.py       # f2 文件哈希缓存（storage/f2_hash_cache.db）
│   │   ├── verify_storage_move.py # 数据盘迁移校验（只读：DB 路径引用可达 + 逐目录比对）
│   │   ├── fetch_xhs_following.py # 拉「我关注的用户」列表（供博主主页补全按昵称解析 uid；也可单独跑出 CSV）
...
│   │   │   ├── f2Authors.ts      # 已登记博主清单表头排序（总作品 / 已入库素材）
│   │   │   ├── f2CollectSelection.ts # 抖音收藏夹勾选记忆（本地只保留最近一次）
│   │   │   ├── faceMedia.ts      # 人脸库扫描页媒体地址（缩略图/大图/头像/组代表图）
│   │   │   └── format.ts         # 字节/耗时/日期格式化
│   │   └── composables/          # Vue composables（useWebSocket / useSearch / useOutfitTags / useTagManage / useAdminTask / useTagAdvanced / useTaskPolling / useFaceServiceStatus / useFaceClusterGroups 等）
│   ├── package.json
│   └── vite.config.ts
│
...
| `inspiration_tags` | 素材-标签关联 | inspiration_id, tag_id, confidence, source（ai_generated/manual/seed） |
| `tag_corrections` | AI 打标纠错记录 | id, inspiration_id（素材删除级联）, tag_id（标签删除置空）, tag_name/category（快照）, reason（multi/missing/wrong_category/bad_name）, action（removed/added/noted）, note, log_id/prompt_version/model_name（冗余当次分析，供按提示词版本聚合纠错率）, created_at |
...
| `bloggers` | 穿搭博主 | id, name, platform, platform_user_id, xhs_id（唯一）, ip_location, profile_url, avatar_path, bio, source, **face_match_threshold（按人自适应人脸匹配阈值，null=用兜底阈值）**, created_at, updated_at |
...
| `collections` | 收藏合集（手动/智能同表，**两级结构**） | id, name（唯一）, description, cover_inspiration_id, position（列表排序）, query_json（null=手动合集；非空=智能合集筛选条件）, kind（manual/smart 由 query_json 区分）, **parent_id（两级：一级固定为「系统入库手动收藏」/「抖音入库自动收藏」；二级=抖音收藏夹，同层唯一）**, **auto_source（douyin=由 f2 入库自动维护，用户自建为 null）**；垃圾桶素材不参与合集内容 |
...
| `inspiration_face_detections` | 素材人脸检测 | id, inspiration_id, face_index（图内序号）, embedding（空=「已扫但无脸」占位，不参与矩阵匹配）, bbox, det_score, confidence（余弦相似度）, matched_blogger_id（命中博主，空为疑似未知人脸）, matched_model_id, match_status（pending/confirmed）, **match_excluded（人工「不匹配」决定，重扫/重匹配后依然生效）**, created_at |
...
>
> **命名合规**：打标命名口径（袜/鞋/裙裸词、丝袜缺长度）统一收敛在 `app/utils/tag_compliance.py`，**打标落库过滤与健康度「不合规命名」扫描共用同一份规则**——新分析不产生裸词，存量裸词可在「标签高级管理 → 健康度」勾选合并到规范标签或删除。

### AI 打标质量闭环

四个环节形成闭环，用于持续提升打标质量（实现见 `app/utils/tag_compliance.py`、`tag_correction_service.py`、`ai_dashboard_service.py`）：

| 环节 | 入口 | 作用 |
| ------ | ------ | ------ |
| 规则兜底 | 打标链路自动 | 落库前丢弃袜/鞋/裙裸词与缺长度丝袜，保证新标签合规 |
| 纠错反馈 | 素材详情「标错了」/「AI 漏标了？补充」 | 把人工判断沉淀为 `tag_corrections` 数据（含当次 prompt 版本） |
| 质量看板 | 数据洞察 → 提示词质量对比 | 按版本对比成功率/平均标签数/纠错率/裸词率，数据化挑选提示词 |
| 规则回归 | `backend/tests/test_tag_rule_regression.py` | 真实模型响应用例锁定规则，改规则即跑，防「改 A 坏 B」 |

> 新增误标案例时，用 `python scripts/export_tag_rule_case.py --log-id <分析日志ID> --case-id <名称>` 从日志导出用例骨架，人工补 `expect.produce` / `expect.drop` 真值后放入 `backend/tests/fixtures/tag_rule_cases/` 即永久生效（不调 Ollama，毫秒级）。
...
| `POST` | `/api/inspirations/{id}/tag-corrections` | 提交 AI 打标纠错反馈（`reason`：multi 删关联 / missing 补关联 / wrong_category / bad_name 仅记录；冗余当次分析的 prompt 版本） |
...
>
> **两级结构**：一级（`parent_id IS NULL`）固定只有两个——「系统入库手动收藏」（智能合集，按 `is_favorite` 求值）与「抖音入库自动收藏」（`auto_source='douyin'`，f2 自动维护）；二级挂在后者之下、名字与抖音收藏夹一一对应，由 f2 入库阶段按「作品 ID → 收藏夹」清单自动建立。**最多两级**（同层唯一约束），创建二级用 `POST` 带 `parent_id`，一级不允许被当作父节点继续套娃；名字口径唯一定义在 `app/models/collection.py`。
...
| `GET` | `/api/tags/health/{issue_type}` | 健康度问题明细（issue_type ∈ orphan/low_frequency/low_quality_name/duplicate/**noncompliant**，分页；noncompliant 返回原因码与中文原因，实时复校过滤已改名标签） |
...
| `POST` | `/api/bloggers/{id}/avatar` | 设置博主头像（multipart，`inspiration_id` 与 `file` 二选一：从 TA 的素材选一张 / 本地上传；取最大人脸裁成 256×256，无脸时回退整张照片，返回 `{avatar_path, face_cropped, message, blogger}`） |
| `DELETE` | `/api/bloggers/{id}/avatar` | 清除博主头像（同时删掉头像文件） |
| `POST` | `/api/bloggers/{id}/promote` | 「自动登记」博主纳入追踪（`source` 改回 manual，之后增量下载才带上 TA 的主页作品） |
| `GET` | `/api/bloggers/missing-profile` | 资料有缺口的博主列表（小红书缺主页链接 / 平台用户 ID，抖音缺 IP 属地；带 `platform` / `platform_user_id` 供前端标注缺什么） |
| `POST` | `/api/bloggers/enrich-missing-profile` | 创建「一键补全主页」异步任务（body `{"blogger_ids": [...]}` 可选；返回 `task_id` / `total`） |
| `POST` | `/api/bloggers/enrich-skip` | 手动跳过指定博主（`{"blogger_ids": [...], "reason": "..."}`，跳过后不再出现在缺口列表） |
| `POST` | `/api/bloggers/enrich-unskip` | 解除跳过（重新纳入补全范围） |
| `GET` | `/api/bloggers/enrich-skips` | 已跳过博主列表（含原因与时间，供界面查看 / 解除） |
| `POST` | `/api/inspirations/{id}/bloggers` | 给素材批量关联博主（幂等） |
| `DELETE` | `/api/inspirations/{id}/bloggers/{bid}` | 解除素材与博主关联 |

> **一键补全主页（按平台分流）：** 人物管理页的「补全主页信息」按平台走两条完全不同的路。
> **抖音**是纯离线——直接读 f2 用户库 `douyin_users.db` 的 `user_info_web`，按 `platform_user_id`
> ↔ `sec_user_id` **精确匹配**回填 `ip_location`，不联网、零风控、**只补空缺**（手填 / CSV 导入的值优先）。
> **小红书**拉一次「我关注的用户」列表（一次请求拿到全部关注账号的 uid），按**昵称归一化**
> （strip + 全角转半角 + 小写 + 去 emoji/符号）匹配出 uid 拼出主页链接，其余情况走本地 URL ↔ ID 互推，
> 因此除这一次请求外逐个博主零请求——没有逐博主的风控延时，也不设单次数量上限。
> **为什么不用「按小红书号搜用户」**：实测小红书用户搜索无法按小红书号定位本人（搜 `1036376990`
> 返回的是名称相近的无关用户，卡片里也没有主页链接可点），只会稳定判成「无法唯一确认」；
> 而素材库里的博主本来就是从这份关注列表导入的，昵称可以一一对应。归一化后**重名的昵称整条剔除**
> （实测关注列表里 `oo` 与 `oo-` 都会归一化成 `oo`）——宁可不填，也不把错误的 uid 写进库。
> 三种查不到的情况（缺 ID / f2 库里没有该账号 / 关注列表里对不上）会记入**已跳过**并写明原因，
> 在界面解除后即可重试；临时性问题（未导入 Cookie、接口被风控拦截）不写跳过表，任务直接失败等重试。
...
| `GET` | `/api/ai/tag-corrections` | 打标纠错记录列表（分页，可按 reason 筛选） |
| `GET` | `/api/ai/tag-corrections/stats` | 纠错统计（按原因/动作/提示词版本聚合） |
| `GET` | `/api/ai/prompt-quality` | 提示词版本质量对比（按版本聚合成功率/平均标签数/纠错率；`include_bare_rate=true` 额外采样重放原始响应算裸词率） |
...
| `POST` | `/api/scraper/f2-tasks/{task_id}/results/trash`<br>`POST .../results/restore`<br>`POST .../results/delete` | 批次结果复核操作：批量移入垃圾桶（`{"ids": [...], "reason": "质量差"}`，软删除且作为负样本）/ 从垃圾桶恢复（`{"ids": [...]}`）/ 彻底删除（`{"ids": [...]}`，转 `batch_delete` 任务由 worker 执行） |
| `POST` | `/api/scraper/f2-tasks/{task_id}/results/register-bloggers` | 把该批次里**未登记**的来源作者补建成抖音博主并绑定本批素材（手动路径，「我的喜欢」入库时默认自动执行同一逻辑） |

> **断点续采：** 失败任务可「续采」，沿用 `resume_token` 中的执行计划（关键词 × 排序）从未完成处继续，已入库图片不重复采集。
>
> **采集 ROI 漏斗（两个通道的口径不同，都不摊派）：** CDP 采集走任务级 `items_found` / `items_added`，合格率靠素材的 `scraper_task_id` **按任务精确归属**；f2 通道没有这个关联（该列外键指向 `scraper_tasks`，塞不下任务队列 id），于是按素材的 `source_author` 直接聚合「入库 / 合格」，因此 f2 行的任务、发现、入库率列显示「—」而不是编一个数字。两条边界会写进 `coverage.notes` 并在界面上展示：**多关键词任务**的采集量无法按词拆分，整体排除；**合格率只算已审核样本**（通过 /(通过+不合格)），没有样本时返回 `null`——界面上「待审」列始终与「合格率(已审)」并排，避免把 98% 读成「全都审核通过」。
>
...
- **过期锁自愈**：上次备份被中断会残留 `backup.lock`，过去会让后续每次备份直接退出（曾整整 7 天没有新备份）。现在按锁目录 mtime 判定，超过 `BACKUP_LOCK_MAX_AGE_HOURS`（默认 6 小时）即判为残留、打印原因后清除继续。
- **存储根跟随配置**：素材备份源与日志目录都按 `settings.storage_root` 解析（不再写死 `backend/storage`）——存储迁盘后不会静默备份旧树、也不会把日志写到旧位置。
...
- **f2 抖音素材通道**：`test_unit_f2_import`（文件名解析/作品分组/报表统计/**五层去重**（含垃圾桶命中与「在库优先于垃圾桶」）/**哈希缓存**（跨运行复用且不重复读盘、文件改动后失效、缓存损坏退化为全量重算）/**日期窗口**（`all` / 最小窗口 / 按作者目录 mtime 放大 / 新博主全量 / 命令透传）/**我的喜欢**（文件名保留原作者与下划线昵称、与发布模式共用作品 key 与平台 ID、与已入库的发布模式素材互相判重、点赞命令形状与命名模板、我的主页链接归一化、缺链接时报错、驱动失败判负）、`test_unit_sync_blogger_ids`（清单解析与脏数据拒绝、预览不改库、更新与新建分支、素材绑定幂等）、`test_f2_import_task`（任务创建与参数透传、执行链路入库与幂等、**二次运行哈希全命中**、**垃圾桶素材不被复活**、下载阶段先写 `stage=download`、恒失败时任务显式失败、规划不阻塞事件循环、并发复用同一任务、**我的喜欢**：参数透传 / 按原作者入库 / 重跑判重 / 缺「我的主页链接」时拒绝 / **下载期实时进度与软进度** / **入库后自动登记博主并绑定素材**（可关闭）/ 发布模式不登记 / 手动登记端点幂等与 404 / 「纳入追踪」后进入下载白名单 / 点赞可用性不受「已登记博主」白名单影响）
- **存储/配置与收藏夹扫描**：`test_unit_config_paths`（`STORAGE_ROOT` / `F2_DOWNLOAD_ROOT` 覆盖后各子目录跟随、测试进程环境变量隔离）、`test_unit_verify_storage_move`（迁移校验脚本：DB 路径引用全部可达、逐目录文件数与字节比对、缺失即报）、`test_unit_f2_merge`（「我的喜欢 / 我的收藏」硬链接合并：内容相同才合并、失败不破坏原文件）、`test_unit_f2_collects`（收藏夹枚举与分页、按夹下载、进度快照不被下载线程改写、已有作品前导过滤）、`test_face_service_status`（子服务离线态：并发展开只发一次请求、失败保持「未知」不误锁人脸功能）
- **博主主页资料补全**：`test_blogger_enrichment`（本地互推 URL↔ID、关注列表按昵称归一化匹配（全角/emoji/大小写差异）、**归一化重名昵称整条剔除**、不在关注列表 → 跳过并给出两条出路、URL 无法解析 → 跳过、缺 Cookie 任务快速失败且写明原因、关注列表接口异常让任务显式失败、跳过/解除跳过往返、处理顺序（抖音离线优先、两项全缺优先）、**抖音离线回填**（只补空缺 / 已填值不覆盖 / f2 库无此账号 / f2 库该账号无属地各自写明原因并写跳过表、幂等重跑为 0）、`parse_xhs_id`（下划线/连字符不截断））、**采集 ROI 漏斗**（`test_collection_roi`：关键词维度 任务级发现/入库与入库率、素材按 `scraper_task_id` 归属的通过/不合格/待审与合格率、**多关键词任务整体排除**并给出说明、无可归属样本时合格率为 `null`、垃圾桶素材退出样本；博主维度 f2 按 `source_author` 聚合且任务级字段为 `null`、CDP 按博主任务用任务口径、博主删除后名字回退 `# id`；统计窗口同时过滤任务与素材、`limit` 逐维度降序截断、越界参数 422 与服务层收敛）、**博主头像**（`test_blogger_avatar`：从已关联素材选图 / 本地上传 / 视频素材取首帧缩略图、取最大人脸裁成方形并校验外扩与边界夹取、无脸与人脸子服务不可用时回退整张照片、素材不属于该博主或来源二选一缺失 → 400、重复设置覆盖旧文件、清除头像、删除博主连带删头像文件）
...
覆盖范围：`format` / `sourceLabel` / `taskLabel` / `browseQuery` / `ipStatsChart`（IP 属地条形图配置：**不隐藏任何一档标签**、高度随条目数增长）纯函数、`taskPresentation`（任务结果汇总 + **运行中阶段文案**：f2 下载/入库/登记博主/排队/暂停 + 「我的喜欢」实时下载计数，非 f2 任务不编造）、`tagHistoryDiff`（操作历史 before/after 差异与值格式化）、`useSplitResize` 拖拽 / `useBatchSelection` 批量多选 composable、`useF2Import`（f2 状态解析 / 自动获取开关 / 一键获取与「我的喜欢」参数透传）、`F2LikeCard`（「我的喜欢」卡片：链接保存 / 实时下载进度文案 / 一键拉取）、`useF2Results` + `F2ResultsPanel`（批次结果分类过滤 / 移入垃圾桶与恢复）、`useF2TaskHistory`、`useScraperRoi`（采集 ROI：按窗口天数请求 / 切窗口重拉 / 空数据判定 / 失败置空 / 口径说明透传，以及 `formatRate`、`formatCount` 对「没有这个口径」与「样本为 0」都显示「—」）、`PersonAvatarPicker`（头像选择器：素材列表加载 / 上传前校验 / 结果提示）、`PersonTopList`（热门排行渲染）、`persons`（博主/模特双 store 实例 + 请求序号防乱序）/ `inspirations` / `tags` store（mock API）、`taskLabel` 任务类型/图标映射断言、`faceMedia`（人脸库扫描媒体地址：`media_type` 缺失时按后缀判视频，**悬停大图绝不返回 mp4**）、`FaceUnmatchedTab`（悬停浮层：停留后弹出、移出消失、大图取原图）、`useFaceServiceStatus`（未知态不误伤 / 探测到不可达带回中文原因 / 请求本身失败不锁死人脸功能 / 并发复用同一次探测）、`f2Authors`（博主清单表头排序：空值与字段缺失当 0 且不抛错、两方向互为相反数）、`f2CollectSelection`（收藏夹选择只保留最近一次、脏数据当无记录、恢复时只勾仍存在的夹并报告失效数量）、`AdminNearDuplicates`（逐组对比：确认删除才 emit、中途退出也提交已做的决定、可翻页改主意、全部跳过不提交空删除、连点保护）。
