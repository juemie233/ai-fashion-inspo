...
### AI 视觉模型配置统一（文档/配置不一致待修）

**背景：** config.py 默认 `ollama_vision_model = "minicpm-v:8b"`；README 前置条件与架构图写 Qwen3-VL:8B-Instruct；CLAUDE.md 技术栈写 MiniCPM-V:8b，三者不一致。用户按 README 装 Qwen3-VL 后，默认配置却调 MiniCPM-V，可能拉取/调用错模型。

**目标：** 统一 config 默认值、README、CLAUDE.md 三处，明确推荐模型与切换方式。

**验收标准：** 三处描述一致，按文档安装的模型即为默认调用模型。

### 破坏性接口认证覆盖（安全加固）

**背景：** auth.py 提供 `require_api_key` 依赖，但仅部分路由启用；`api_key` 默认空 = 跳过认证（开发模式）。破坏性接口（数据重置、批量删除、清空垃圾桶、去重删除等）若暴露在局域网/外网，存在误操作或未授权风险。

**目标：**

- 破坏性接口统一挂 `require_api_key`，读接口保持开放
- 首次启用时提供 api_key 生成引导

**验收标准：**

- 设置 api_key 后，破坏性接口无密钥一律 401/403
- 读接口不受影响
