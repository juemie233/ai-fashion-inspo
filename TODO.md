...
**描述：** 后端 schema 变更后若未重启，前端会静默降级（如漏斗按钮消失、字段拿不到），用户无从察觉。通过版本握手让前后端在启动时对一次"版本暗号"，不一致则明确提示，把"静默失败"变成"显式提示"。

**目标：**

- 前端启动时请求 `/api/health`，比对后端 `schema_version` 与本地期望值
- 不一致时弹出明显提示："后端 API 版本不匹配，请重启后端"，而不是静默出错

**实施步骤：**

1. **后端暴露版本号** — `/api/health` 返回 `schema_version` 字段，每次改数据库结构（加列/改字段）时手动 +1
2. **前端维护期望版本** — 前端代码硬编码一个常量，表示前端依赖的 schema 版本，跟着后端一起改
3. **启动时校验** — 前端应用启动时请求 health，比对版本号，不一致则拦截并提示
4. **不匹配提示 UI** — 在全局布局加横幅/弹窗，明确告知用户如何处理
...
- 版本号自动计算：后端启动时根据 `db_migrations.py` 的字段清单内容计算哈希作为版本号，避免"改了字段清单但忘了加版本号"的疏漏。第一版可先手动维护。
