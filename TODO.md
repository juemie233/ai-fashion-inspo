...
**描述：** 当前采集流程是"全部提取完 → 统一下载"，URL 签名可能在提取与下载之间过期。改为流水线模式：边滚动边提取边下载，缩短 URL 从提取到下载的时间窗口。

**方案：**
- 每轮滚动后立即下载新提取的 URL，而非等所有滚动完成
- 使用生产者-消费者队列（`queue.Queue`），提取线程生产 URL，下载线程消费
- 减少 URL 签名过期的损失
...
**描述：** 采集任务完成后，在 Web 页面展示采集到的图片缩略图网格，用户可以快速预览并批量删除不需要的图片。

**方案：**
- 采集完成后展示"本次采集结果"缩略图网格
- 支持点击选中/取消选中，批量删除
- 支持"全选非模特照片"等智能筛选

**涉及模块：**
- `web/src/views/ScraperView.vue`（采集管理页面）
- 后端新增 `GET /api/scraper/tasks/{id}/results` 接口
