...
**背景：** 代码审查发现：现有测试多为「单环节」测试（验证环节内部逻辑），验证不了「环节衔接」——字段没传、枚举不一致、事务边界错位这类跨模块隐形 bug 只在全旅程中暴露。垃圾桶/采集/失败恢复三条主链路尚无端到端覆盖。

**目标：**

- 素材旅程：上传 → 打标 → 向量 → 移入垃圾桶 → 恢复 → 再移入 → 清空，全程断言状态机合法
- 采集旅程：采集入库 → 删除 → 墓碑生效 → 重采被拒
- 失败旅程：Ollama 假死 / 文件缺失 / 进程崩溃后重跑，验证任务自愈与防假成功

**涉及模块：**

| 模块 | 改动 |
| ------ | ------ |
| `backend/tests/test_trash.py` 等 | 新增全旅程测试（复用现有 upload/ollama 故障 fixture） |

**验收标准：**

- 三条全旅程测试通过，且任一步骤破坏状态机/墓碑/自愈语义时测试变红
- 单测保环节、旅程测试保衔接，各 3~5 条不贪多，避免补丁式集成测试拖慢 CI

**风险与依赖：**

- 维护成本是持续负担：链路每次变化需同步更新测试，否则测试本身变噪音
- 故障注入需 mock Ollama/LanceDB/进程，mock 与真实行为偏差可能引入误报

### 幂等与数据库唯一约束（方案 C）

**背景：** 任务重试、用户双击、插件重发等「重复执行」是隐形 bug 的重要来源；当前逻辑层有查重，但缺少数据库级兜底。

**目标：**

- `scraper_seen_urls`（墓碑）加 UNIQUE 约束，杜绝重复采集
- `inspirations.source_platform_id` 加部分唯一索引（仅 `deleted_at IS NULL` 的未删除素材），防止重复入库；**注意保留现有语义：垃圾桶素材释放平台 ID（软删除后可重新入库）**
- 任务执行器补幂等断言：同一任务重跑，结果与统计不变

**涉及模块：**

| 模块 | 改动 |
| ------ | ------ |
| `backend/app/models/` + Alembic 迁移 | 唯一约束/部分索引 |
| `backend/tests/test_task_runners.py` | 幂等断言测试 |

**验收标准：**

- 重复采集/重复入库被数据库拒绝（而非静默重复）
- 已有重复数据清理后迁移成功（先清数据再上约束）

**风险与依赖：**

- ⚠️ 平台 ID 部分唯一索引与「垃圾桶释放平台 ID」语义有交互，迁移前需评估存量重复数据并清洗
- SQLite 部分唯一索引（`WHERE deleted_at IS NULL`）支持但需验证与现有查询兼容
