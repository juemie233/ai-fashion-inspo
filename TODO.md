...
### 补全向量检索能力，实现语义搜索与以图搜图

**背景：** 当前相似素材推荐可能基于标签匹配，无法捕捉视觉风格相似性。embedding_service.py 已存在但未投入使用。

**目标：** 引入本地向量数据库，实现视觉相似搜索，提升搜索体验。

- 评估并引入向量数据库（推荐 LanceDB，嵌入式，无需额外服务）
- 实现图片向量生成：
- 使用 Ollama embedding 模型（如 nomic-embed-text）或 CLIP 模型
- 在 embedding_service.py 中完成图片预处理与向量提取
- 为存量素材批量生成向量（可通过后台任务队列执行）
- 新增 API 端点：
- POST /api/search/vector：接受文本或图片，返回 TopK 相似素材
- GET /api/search/similar/{id} 改为使用向量相似度，并混合标签匹配加权排序
- 前端集成：
- 素材详情页"相似素材推荐"标注相似来源（视觉/标签）
- 搜索页增加"以图搜图"上传入口
- 新上传素材自动触发向量生成（可异步）

**验收标准：**

- 上传一张穿搭图，能够返回视觉上相似的素材（不依赖相同标签）
- 相似搜索响应时间 < 500ms（本地环境）
- 向量数据库文件可随项目迁移
